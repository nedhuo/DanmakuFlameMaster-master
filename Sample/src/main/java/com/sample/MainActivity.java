...
public class MainActivity extends Activity implements View.OnClickListener {
    ...
    private BaseDanmakuParser createParser(InputStream stream) {
        ...
        ILoader loader = DanmakuLoaderFactory.create(DanmakuLoaderFactory.TAG_LONGCHAT);
        try {
            loader.load(stream);
...
//        parser.load(dataSource);
        LongChatDanmuParser parser = new LongChatDanmuParser();
        return parser;
...
//        findViewById(R.id.btn_lineCount).setOnClickListener(v -> {
//
//        });
...
//        .setCacheStuffer(new BackgroundCacheStuffer())  // 绘制背景使用BackgroundCacheStuffer
                ...
                .setDanmakuMargin(40);
        if (mDanmakuView != null) {
            mParser = createParser(this.getResources().openRawResource(R.raw.comments)); //弹幕资源
...
//            mDanmakuView.enableDanmakuDrawingCache(true);
    ...
    private void addDanmaku(boolean islive) {
        BaseDanmaku danmaku = mContext.mDanmakuFactory.createDanmaku(5);
        if (danmaku == null || mDanmakuView == null) {
            return;
        }
        // for(int i=0;i<100;i++){
        // }
        danmaku.text = "指定弹幕" + System.nanoTime();
        danmaku.padding = 5;
